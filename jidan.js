 ...
 * ---------------------------------------------------------------------------------------------------------
 ...
 /**
  * 打卡    httpPost
  * https://act.you.163.com/act/napi/play/web/activation/sign/query.json
  */
 async function checkIn() {
     let Options = {
         url: `${hostname}/act/napi/play/web/activation/sign/try.json`,
         headers: {
             'Host': host,
             'Cookie': ck[0]
         },
     ...
     let result = await httpPost(Options, `打卡`);
 
     if (result.code == 200) {
         DoubleLog(`打卡: 成功 🎉`);
     }  else {
         DoubleLog(`打卡: 失败 ❌ 了呢,原因未知!`);
         console.log(result);
     }
 }
